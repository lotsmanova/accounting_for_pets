...
async def add_pet(pet: PetAdd, session: AsyncSession = Depends(get_async_session)) -> PetRead:
...
async def get_pets(
        has_photos: Optional[bool] | None = None,
...
async def delete_pet(id_list: List[uuid.UUID], session: AsyncSession = Depends(get_async_session)) -> dict:
    pet_del = await pet_crud.delete_pet(session, id_list)
    return pet_del


@router.post("/{id}/photo", response_model=PhotoRead)
async def add_photo(id_pet: uuid.UUID, file: UploadFile, session: AsyncSession = Depends(get_async_session)) -> PhotoRead:
